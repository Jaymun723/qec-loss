...
void pybind_monaka_builder(py::module &m) {
    py::class_<MonakaBuilder>(
        m, "MonakaBuilder",
        R"doc(Builder to analyze and compile detector error models under loss.)doc")
        .def(py::init<const LossyCircuit &, bool>(), py::arg("circuit"),
             py::arg("optimize_rerouting") = false,
             R"doc(Initialize MonakaBuilder with a LossyCircuit.)doc")
        .def(
            "get_nominal_dem",
    ...
    py::class_<LifeSegment>(
        m, "LifeSegment",
        R"doc(Represents a continuous segment of a qubit's lifecycle.)doc")
        .def(py::init<uint32_t, size_t, size_t>(), py::arg("qubit"),
             py::arg("start"), py::arg("end"),
             R"doc(Initialize LifeSegment.)doc")
    ...
    py::class_<LifeCycleManager>(
        m, "LifeCycleManager",
        R"doc(Manages and resolves lifecycles of qubits.)doc")
        .def(py::init<const LossyCircuit &>(), py::arg("circuit"))
        .def("get_life_cycle", &LifeCycleManager::get_life_cycle,
             R"doc(Get the full lifecycle of a given qubit.)doc")
        .def("get_life_segment_for_measurement",
             &LifeCycleManager::get_life_segment_for_measurement,
             R"doc(Get the life segment associated with a measurement.)doc");
    ...
    m.def(
        ...
        [](const LossyCircuit &circuit, const LifeSegment &life_segment) {
            std::vector<stim::Circuit> result =
